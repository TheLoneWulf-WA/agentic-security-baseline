...
- **`hooks/push-routing-gate.sh`** — A Claude Code PreToolUse hook. Fires on every `git push` Claude tries to run (prompts when a protected branch — `main`, `master`, `production` — appears as a push target, including refspec forms like `HEAD:main`), on PR merges via the routes agents actually use — `gh pr merge`, the `gh api` merge endpoint, `curl`/`wget` to the GitHub API — and on shell commands that write to the enforcement config. Merges pause for a one-keypress confirmation ("ask mode"), and the prompt shows the review-loop marker state for the current branch — clean, stale, or missing. The `"ship it through"` bypass phrase skips the prompt and every use of it is written to an audit log. One word in the script flips merges to "deny mode" if you'd rather click merge in the GitHub UI yourself. It's a grep on command shapes, not a sandbox: an unenumerated route (a script file, a non-curl HTTP client) walks past it — see the honest-limits note below.
- **`hooks/config-edit-gate.sh`** — The guard guarding the guard: a PreToolUse hook on `Edit`/`Write`/`NotebookEdit` that prompts whenever the agent tries to edit the enforcement config itself — hooks, settings, the protocol `CLAUDE.md`, the skills and commands the loop depends on. Resolves symlinks and relative paths before matching. A gate an agent can silently rewrite is not a gate. (The platform's own permission classifier also blocks some of these edits — this hook makes the protection explicit and yours.)
- **`skills/until-clean/SKILL.md`** — A slash command (`/until-clean`) that mechanizes the review-until-clean loop: re-reviews the current branch's PR, verifies every previously raised finding is actually resolved (not reworded), loops fix → re-review until a pass comes back empty, then reports one fixed-format verdict line and writes a per-branch clean marker (`<git-dir>/review-clean/<branch with / replaced by _>`) recording the head SHA of the clean pass. Exists because "the findings were fixed" and "a re-run came back clean" are different claims — the gap between them is where I kept drifting.
...
- **A skill** (`/until-clean`) that runs the whole loop as a procedure with a fixed-format answer — `Review loop: CLEAN (re-run after fixes at <sha>)` or a findings list. One short command replaces a question I was typing over and over.
- **A marker** written on each clean pass (`<git-dir>/review-clean/<branch with / replaced by _>`, head SHA + timestamp). The merge gate reads it: its confirmation prompt tells you whether the current branch's marker matches HEAD (clean), predates it (stale — commits landed after the clean pass, the exact drift class above), or doesn't exist. The comparison is scoped to the current branch, so if you're merging a different branch's PR, the verdict line in the agent's pre-merge summary is your evidence instead.
